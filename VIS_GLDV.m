...
for difference_r=1:imager
    for difference_c=1:imagec
        if (difference_c+2<=imagec)
            difference_mat(difference_r,difference_c)=abs(vis_mat(difference_r,difference_c)-vis_mat(difference_r,difference_c+2));
        else
...
for a=1:4
    for b=1:4
        ...
        for m=1:freq_matr_4x4
                cpvector((a-1)*4+b,1)=cpvector((a-1)*4+b,1)+(((freq_mat_4x4(m,1)-meanvector((a-1)*4+b,1))^4)*(freq_mat_4x4(m,2)/tot_freq_4x4));
        end
